...
function App() {
  ...
  const { licenseActivated, user, isAuthenticated } = useAuthStore();
  ...
  const isAccessGranted = licenseActivated && !!user && isAuthenticated;
  ...
  return (
    ...
    >
      ...
      {/* z-10 keeps in-app modals/sheets above the global keyboard hint strip (fixed, later in DOM). */}
      ...
      {/* Keyboard hints — single chip row, z-0 under main (see flex wrapper). */}
